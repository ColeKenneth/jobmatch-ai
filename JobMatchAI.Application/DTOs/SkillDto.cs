...
namespace JobMatchAI.Application.DTOs
{
    public record SkillDto
    {
        ...
        [MaxLength(length: 100, ErrorMessage = "Skill name cannot exceed 100 characters.")]
        public string Name { get; init; } = string.Empty;

        [EnumDataType(typeof(Category), ErrorMessage = "Invalid category.")]
        public Category Category { get; init; }

        [MaxLength(length: 100, ErrorMessage = "Ontology ID cannot exceed 100 characters.")]
        public string? OntologyId { get; init; }

        public string? Description { get; init; }

        [MaxLength(length: 500, ErrorMessage = "Icon URL cannot exceed 500 characters.")]
